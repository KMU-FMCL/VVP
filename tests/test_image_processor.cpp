    ...
    void SetUp() override {
        ...
        for (int y = 0; y < testImage.rows; y++) {
            for (int x = 0; x < testImage.cols; x++) {
                ...
                testImage.at<cv::Vec3b>(y, x) = cv::Vec3b(50, 50, x * 255 / testImage.cols);
            }
        }
        
        // 세로 선 추가 (90도 방향 강조)
        cv::line(testImage, cv::Point(50, 0), cv::Point(50, 100), cv::Scalar(255, 255, 255), 2);
    }
...
TEST_F(ImageProcessorTest, RotateImage) {
    ...
    EXPECT_LT(diff / (testImage.rows * testImage.cols), 1.0); // 평균 차이가 작아야 함
}
