...
class Runner::Impl {
 public:
  ...
  auto Run() const -> int {
    ...
    absl::Span<char*> args(argv_, argc_);
    ...
    // 입출력 핸들러 초기화
    vv::io::InputHandler input_handler(config);
    vv::io::OutputHandler output_handler(config);
    absl::Status status = input_handler.open_video_source();
    if (!status.ok()) {
    ...
    vv::VVHistogramVisualizer vv_visualizer(cfg_all.vv);
    ...
    cv::Mat frame;
    status = input_handler.read_next_frame(frame);
    if (!status.ok()) {
      std::cerr << "Error: " << status.message() << '\n';
      return 1;
    }
    ...
    while (true) {
      ...
      double fps_to_record = fps_counter.get_fps();
      ...
      // 이미지 회전 (보정)
      cv::Mat calibrated_image = vv::utils::rotate_image(
      ...
      // 시각화 컨텍스트 생성 및 데이터 설정
      vv::visualization::VisualizationContext viz_context;
      ...
      viz_context.setFps(fps_counter.get_fps());

      // 시각화 이미지 생성 (컨텍스트 사용)
      cv::Mat visualization_result =
          vv::visualization::create_visualization(viz_context);

      // 결과 표시 및 저장
      int key = vv::utils::DisplayUtils::display_frame(
          "Visual Vertical Estimation", visualization_result, 1);
      output_handler.write_frame(visualization_result);
    ...
    if (config.save_results) {
      status =
          output_handler.save_results_to_csv(vv_estimator.get_all_results());
      if (!status.ok()) {
